...
already reached writes `1`. Measured on the HUB Navizone (1758) and its three
rooms, from a diagnostics dump of 2026-09-25 : the vendor app wrote the
window (222 and every room's 100260) at 10:19:02 for a start at 10:28:49, and
152 with every room's 100261 changed at 10:29:07 -- eighteen seconds after
the start, which nobody does by hand. So the cloud turns the absence on when
its start comes. What the app wrote into 152 at 10:19 was overwritten before
the dump and is not known ; `2` is the one value the catalogue gives an
absence waiting for its start, and writing `1` early is what the measurement
says the cloud does itself, later. A switch reading 2 counts as on.
...
first sensor read, and kept whatever they held after that : the same dump's
absence had been set from the vendor app, and the pickers showed a window
four days wide that was not it.
...
same 1790238529 -- 10:28:49 in Paris, the morning the vendor app set it --
